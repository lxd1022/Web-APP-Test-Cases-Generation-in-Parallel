...
class EFSM:
    ...
    def __init__(self, name, transitionList=[]):
        ...
        self.max = 8191
        self.condlist = {}
        self.TEvent = {}   ##store all transition's event for create test script 字典存储
    ...
    def deletefile(self):
        #file = 'D:/wamp_php/wamp/www/schoolmate2/muser_result.txt'武淑美9/11修改
        file = 'D:/programs/wamp/www/schoolmate2/muser_result.txt'
    ...
    def obtainIndividualFitness(self,currPathT, invidual,varname, populationSize, unTvardata):
        ...
        for i in range(populationSize):
            ...
            # 把self.TEvent 传入测试脚本生成部分，指导自动构建脚本
            testcase_seq.runcase.seq_script(self.TEvent,currPathT)
    ...
    def testdate_relate(self, currPathT, unTvardata):
        ...
        self.currPathTranVarDict = {}
        pathVarType2 = []
        matrix = [[0 for i in range(2)] for i in range(populationSize)]
        self.copyPathInfo()
        ...
        if len(self.originalDef) > 0:  ##There exist input variables on the path
            for repeat in range(10):
                ...
                while j <= 10000:
                    ...
                    # 计算种群个体适应度值
                    oldInvidualFit, oldtargetfit = self.obtainIndividualFitness(currPathT, population, varname, populationSize, unTvardata)
                   ...
                   #  oldInvidualFit = {0: 0.1667, 1: 0.2667, 2: 0.3667, 3: 0.5667, 4: 0.4667, 5: 0.1667, 6: 0.1667, 7: 0.1667, 8: 0.1667, 9: 0.1667}
                    ...
                    #      temp_pop = gaSample.creatStartPopulation(pathVarType2)
                       ...
                       #  计算一下变异后的种群个体的适应度值
                         newInvidualFit = self.obtainIndividualFitness(currPathT, temp_pop, varname, populationSize,unTvardata)
