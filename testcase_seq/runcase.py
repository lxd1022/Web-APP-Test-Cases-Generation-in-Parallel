...
def runtestcase(unvardata, invidual):
   ...
   #######################################################################################################################
   driver.get("http://localhost/openconf/")
   #driver.get("http://localhost/1addressbook/")
   #执行封装的函数
...
if __name__ == '__main__':
    ...
    driver.implicitly_wait(30)
    ####################################################################################################################################################################
    driver.get("http://localhost/openconf/")#武淑美9/19修改2
    #driver.get("http://localhost/1addressbook/")#武淑美9/10修改2
    # 执行封装的函数
    case.dologin(driver, u"test", u"test")  # T3
    #case.click_user(driver)  # T4
    #case.delete_user(driver)
    #driver = 1
    #pathT=['T3', 'T101', 'T102', 'T104']#adressbook时有
    tevent = "模型的迁移序列集"#adreessbook时有
    # print tevent
    #induval = ['test','1',"abll",'ablfc']  #个体变量的数据，产生关于用户名密码的统统人为化，一遍得以进入系统.adressbook时有
