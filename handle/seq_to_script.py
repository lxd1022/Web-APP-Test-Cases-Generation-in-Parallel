...
def seq_script_webchess1(driver,tevent,pathT,induval):
    infor = []
    # print "pathT\n", pathT
    # print "tevent\n", tevent
    for T in pathT:
...
def seq_script_addressbook(driver,tevent,pathT,induval):
    ...
    for info in range(len(infor)):
        ...
        T_info = T_info.split(";")
        if T_info[0] =="" and T_info[2] == "click":
            T_index= T_info[1].find("=")
            temp=[]
            temp.append(T_info[1][:T_index])
            temp.append(T_info[1][T_index+1:])
            if temp[0] == "xpath": # 对象类型
                case.click_event_xpath(driver, temp[1])
            elif temp[0] == "css":
                case.click_event_css(driver,temp[1])
    ...
    for info in range(len(infor)):
        ...
        if T_info[0] =="" and T_info[2] == "click":
            ...
            if temp[0] == "name" and len(t) == 1:
                driver.switch_to_frame("mainFrame")
                if tt=="T62": #数据恢复
                    url = "D:\\PyCharm Community Edition 2017.2\\projects\\graphTraversal\\support\\20180921_all.sql"
                    case.T62(driver)
                else:
                    url="D:\\PyCharm Community Edition 2017.2\\projects\\graphTraversal\\support\\888.png" #上传
                case.php_upload(driver, t[0], url, temp[1])
                driver.switch_to.default_content()
    case.doquit(driver)

def seq_script_openconf(driver,tevent,pathT,induval):
    infor = []
    for T in pathT:
        infor.append(tevent[T])
    j = 0
    for info in range(len(infor)):
        ...
        if T_info[0] =="" and T_info[2] == "click":
            temp = T_info[1].split("=")
            if len(temp)>2 :
                for tp in range(len(temp)-2):
                    temp[tp + 2] = '='+ temp[tp + 2]
                    temp[1] += temp[tp + 2]
            case.NO_input_click(driver, temp[0], temp[1])
        elif T_info[0] != "" and T_info[2] == "click":
            # 属于有输入变量的click事件,call click_event_input()
            t = re.findall('[^()]+', T_info[0])[1]  # 提取输入变量
            t = t.split(",")
            ...
            if len(temp)>2 :
                for tp in range(len(temp)-2):
                    temp[1] += '=' + temp[tp + 2]
            if tt=="T5":
                #http://localhost/openconf/author/edit.php
                case.openconf_Two_Login(driver, t[0], t[1], "1", "test12345", temp[0], temp[1])
            elif tt == "T9":
                #http://localhost/openconf/author/status.php
                case.openconf_Two_Login(driver, t[0], t[1], "1", "test12345", temp[0], temp[1])
            elif tt=="T11":
                #http://localhost/openconf/review/signin.php
                case.openconf_Two_Login(driver, t[0], t[1], "test1", "test12345", temp[0], temp[1])
            elif tt == "T21":
                #http://localhost/openconf/chair/signin.php
                case.openconf_Two_Login(driver,t[0], t[1], "chair", "chair12345", temp[0], temp[1])
            elif tt =="T7":
                case.openconf_Two_Login(driver, "password1", "password2", "test12345", "test12345", temp[0], temp[1])
            else :
                case.openconf_Ninput(driver, t, induval, j, temp[0], temp[1])
    case.doquit(driver)
...
def runcase(tevent, pathT, induval):
    ...
    # openconfig程序需要登录
    #seq_script_openconf(driver,tevent,pathT,induval)

    endTime = datetime.now()
    usertime = endTime - startTime
    print 'endTime - startTime运行该测试用例用时:\t', endTime - startTime
    recordFun.recordTestCaseRunTime(len(pathT),usertime)
...
if __name__ == '__main__':
    ...
    pathT= ['T20', 'T21', 'T124', 'T125', 'T55', 'T56', 'T54', 'T54']
    induval=['KoQkgC', 'PvsBDe', 'zkrMOq', 'xpGXpl', 'JPhrAp', 'wMPjCH', 'SnKtCt']
    ...
    #addressbook
    #pathT=['T2', 'T64', 'T27', 'T28', 'T29', 'T78']#addressbook
