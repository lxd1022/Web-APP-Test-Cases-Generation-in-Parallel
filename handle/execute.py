...
def table_handle(M):
    #通过M获得所有敏感路径入口点覆盖标识cover_flag及敏感路径完全覆盖标识flag
...
def table_coverage(M):#M是评估矩阵，基于M获取违背覆盖的敏感路径集Uncover——path——list）
...
def gdata(seq,SM): # 为序列产生数据，执行测试用例，获取评估矩阵M
    d = []
    M = []
    V = []
    i = 1
    for path in seq:
        ...
        filepath, file2path = config.getInstrumentFile()  # 获取插装生成的信息文件
        if os.path.exists(filepath):
            file = open(filepath, "r+")
            file.truncate()
            file.close()
        data_result, test_fit, coverage,allVar = generate_data_ga.testProcee(SM, path)
